...
This builds the collector image and brings up fake Kasa device emulators
(`harness/fake_kasa.py`, speaking the real IOT protocol) alongside an ephemeral
InfluxDB on a bridge network, then asserts that emeter data for the emulated devices
lands in the `emeter` measurement before tearing everything down. No physical hardware
and no published host ports are required, so it is safe to run in CI or on a busy host.

The harness covers all the fake device kinds: emeter plugs (HS110 / KP115), a
non-emeter plug (HS103), and a 6-outlet power strip (HS300). See
[docs/TESTING.md](../TESTING.md) for details.

For unit tests and linting, use `make test`, `make lint`, or `make check`.

### Q: How do I try the full system interactively?

`make demo-up` brings up an all-in-one stack with **fake** emulated devices (no
hardware): the collector plus a bundled, auto-provisioned InfluxDB and Grafana, so the
dashboards populate immediately. To run that same bundled stack against **your real**
Kasa devices, use `make dev-up` instead. Either way, open http://localhost:3000
(admin / admin); stop with `make demo-down` / `make dev-down`. Run `make help` to list
